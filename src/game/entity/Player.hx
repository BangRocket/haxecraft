...
import game.screen.InventoryMenu;
...
class Player extends Mob {
	...
	override public function tick() {
		...
		// Gamepad LB/RB hotbar cycling
		if (input.hotbarLeft.clicked) {
			selectHotbar((hotbarSelection + 7) % 8);
		}
		if (input.hotbarRight.clicked) {
			selectHotbar((hotbarSelection + 1) % 8);
		}
		if (attackTime > 0) attackTime--;
	}

	private function selectHotbar(i:Int) {
		hotbarSelection = i;
	...
	override public function render(screen:Screen) {
		...
		if (dir > 1) {
			...
			flip2 = flip1;
			// % 2 == 0 means legs together, % 2 == 1 means legs apart
			xt += 4 + (Std.int(walkDist >> 3) % 2) * 2;
		}
		...
		if (isSwimming()) {
			...
			screen.renderSprite(xo + 0, yo + 3, SpriteNames.UI_WATER_DROPLET, waterColor, 0);
			screen.renderSprite(xo + 8, yo + 3, SpriteNames.UI_WATER_DROPLET, waterColor, 1);
		}

		if (attackTime > 0 && attackDir == 1) {
			screen.renderSprite(xo + 0, yo - 4, SpriteNames.UI_ATTACK_INDICATOR_UPDOWN, Color.get(-1, 555, 555, 555), 0);
			screen.renderSprite(xo + 8, yo - 4, SpriteNames.UI_ATTACK_INDICATOR_UPDOWN, Color.get(-1, 555, 555, 555), 1);
			if (attackItem != null) {
		...
		screen.renderSprite(xo + 8 * flip1, yo + 0, SpriteNames.playerRawTile(xt + yt * 32), col, flip1);
		screen.renderSprite(xo + 8 - 8 * flip1, yo + 0, SpriteNames.playerRawTile(xt + 1 + yt * 32), col, flip1);
		if (!isSwimming()) {
			screen.renderSprite(xo + 8 * flip2, yo + 8, SpriteNames.playerRawTile(xt + (yt + 1) * 32), col, flip2);
			screen.renderSprite(xo + 8 - 8 * flip2, yo + 8, SpriteNames.playerRawTile(xt + 1 + (yt + 1) * 32), col, flip2);
		}

		if (attackTime > 0 && attackDir == 2) {
			screen.renderSprite(xo - 4, yo, SpriteNames.UI_ATTACK_INDICATOR_LEFTRIGHT, Color.get(-1, 555, 555, 555), 1);
			screen.renderSprite(xo - 4, yo + 8, SpriteNames.UI_ATTACK_INDICATOR_LEFTRIGHT, Color.get(-1, 555, 555, 555), 3);
			if (attackItem != null) {
				attackItem.renderIcon(screen, xo - 4, yo + 4);
			}
		}
		if (attackTime > 0 && attackDir == 3) {
			screen.renderSprite(xo + 8 + 4, yo, SpriteNames.UI_ATTACK_INDICATOR_LEFTRIGHT, Color.get(-1, 555, 555, 555), 0);
			screen.renderSprite(xo + 8 + 4, yo + 8, SpriteNames.UI_ATTACK_INDICATOR_LEFTRIGHT, Color.get(-1, 555, 555, 555), 2);
			if (attackItem != null) {
				attackItem.renderIcon(screen, xo + 8 + 4, yo + 4);
			}
		}
		if (attackTime > 0 && attackDir == 0) {
			screen.renderSprite(xo + 0, yo + 8 + 4, SpriteNames.UI_ATTACK_INDICATOR_UPDOWN, Color.get(-1, 555, 555, 555), 2);
			screen.renderSprite(xo + 8, yo + 8 + 4, SpriteNames.UI_ATTACK_INDICATOR_UPDOWN, Color.get(-1, 555, 555, 555), 3);
			if (attackItem != null) {
